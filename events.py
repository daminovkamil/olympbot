...
logging.basicConfig(filename="output/events.log", filemode="w")


def ping_admin(text="Советую посмотреть логи) Ошибка в event.py"):
    ...
    try:
        ...
        for activity_id in run(database.fetch("SELECT activity_id FROM cool_olympiads")):
            try:
                ...
                logging.exception(f"ACTIVITY_ID = f{activity_id}")
                logging.exception(error)
                ping_admin(f"Проблемы с activity_id = f{activity_id}")
